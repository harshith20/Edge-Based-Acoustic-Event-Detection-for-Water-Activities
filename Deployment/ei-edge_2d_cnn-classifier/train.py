...
def train_model(train_dataset, validation_dataset, input_length, callbacks,
    ...
    sa = SpecAugment(spectrogram_shape=[int(input_length / 13), 13], mF_num_freq_masks=0, F_freq_mask_max_consecutive=0, mT_num_time_masks=3, T_time_mask_max_consecutive=3, enable_time_warp=False, W_time_warp_max_distance=6, mask_with_mean=False)
...
def main_function():
    ...
    if mode == 'object-detection':
        if input.objectDetectionLastLayer == 'fomo':
            ...
            raise Exception(f'Can only handle objectDetectionLastLayer "fomo" (was "{input.objectDetectionLastLayer}")')
    else:
