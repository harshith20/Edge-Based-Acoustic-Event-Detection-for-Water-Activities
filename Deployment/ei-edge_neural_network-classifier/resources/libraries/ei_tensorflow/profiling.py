...
from ei_shared.types import ClassificationMode, ObjectDetectionDetails, CustomModelVariantInfo, LabelMapMetadata
...
def tflite_predict_yolov11(model, is_coord_normalized, validation_dataset, dataset_length):
    """Runs a TensorFlow Lite model across a set of inputs"""
    interpreter = tf.lite.Interpreter(model_content=model)
    interpreter.allocate_tensors()
    ...
    for batch, _ in validation_dataset.take(-1):
        for item in batch:
            ...
            output = interpreter.get_tensor(output_details[0]['index'])
            output = np.array(ei_tensorflow.inference.process_output(output_details, output))
            # expects to have batch dim here, eg (1, 5, 189)
            # if not, then add batch dim
            if len(output.shape) == 2:
                output = np.expand_dims(output, axis=0)
            rect_label_scores = ei_tensorflow.inference.process_output_yolov11(output, img_size=width,
                                                                               is_coord_normalized=is_coord_normalized)
            pred_y.append(rect_label_scores)
            # Print an update at least every 10 seconds
            current_time = time.time()
            if last_log + 10 < current_time:
...
def make_predictions_tflite(mode: ClassificationMode, model, x_dataset, y, num_classes,
    ...
    if mode == 'object-detection':
        ...
        if objdet_details.last_layer == 'mobilenet-ssd':
            ...
            return tflite_predict_yolov11(model, True, x_dataset, len(y))
        elif objdet_details.last_layer == 'yolov11-abs':
            return tflite_predict_yolov11(model, False, x_dataset, len(y))
        else:
...
def profile_model(model_type: Literal['float32', 'int8', 'akida'],
    ...
    if mode == 'anomaly-gmm':
        # note: a suggested threshold is calculated during training of GMM models and stored as metadata
        # (see GaussianMixtureAnomalyScorer.save_model), so we don't need to calculate it here
        pass
    elif mode == 'anomaly':
        # note: the validation_dataset is in fact the training dataset for AD.
        # this may change in the future if a proper validation split is supported.
        output_directory = os.path.dirname(os.path.realpath(model_path))
        scores = make_predictions_tflite(mode, model, validation_dataset,
                                            Y_test, num_classes, output_directory,
                                            item_feature_axes, objdet_details)
        suggested_threshold = max(0.1, np.percentile(scores, 99))
    elif mode == 'visual-anomaly':
        # first check if this is a built-in visual AD model, because if so there's already
        # a suggested threshold calculated during training (and generating one here would
        # need both feature extractor + scorer head)
        output_directory = os.path.dirname(os.path.realpath(model_path))
        has_ad_scorer = has_ad_scorer_file(output_directory)
        if not has_ad_scorer and len(validation_dataset) > 0:
            # ok it's a custom visual AD model, calculate a suggested threshold based on training scores.

            # note: the validation_dataset is in fact the training dataset for AD.
            # this may change in the future if a proper validation split is supported.
            raw_visual_ad_scores = tflite_predict(model, validation_dataset, len(Y_test), item_feature_axes)
            flattened_scores = np.concatenate([np.asarray(s) for s in raw_visual_ad_scores])
            suggested_threshold = max(0.1, np.percentile(flattened_scores, 99))
    else:
    ...
    evaluator = Evaluator(
        per_sample_metadata,
        sample_id_details["validation"] if sample_id_details else None,
        model_type=model_type,
        dataset='validation',
        is_multilabel = True if label_map_metadata is not None else False
    )

    if label_map_metadata is not None:
        # Make predictions directly on structured data
        if has_samples:
            prediction_samples_structured = feature_explorer_predictions_label_map(
                model,
                akida_model_path,
                X_samples,
                item_feature_axes,
        ...
        prediction_samples_structured = None
    ...
    if mode == 'classification':
        ...
        if not is_custom_variant and X_samples is not None:
        ...
        if label_map_metadata is None:
            # Unbatch predictions before passing in
            unbatched_preds = prediction[:, 0]
        else:
            unbatched_preds = prediction
        eval_result = evaluator.regression(Y_test, unbatched_preds)
    ...
    elif mode == 'anomaly-gmm' or mode == 'visual-anomaly' or mode == 'anomaly':
    ...
    model_info = {
        ...
        'predictionsStructured': prediction_samples_structured,
...
def feature_explorer_predictions_label_map(model, akida_model_path, X_samples, item_feature_axes, input_dir):
    try:
        # Make predictions for feature explorer
        if model:
            feature_explorer_predictions = tflite_predict(model, X_samples, len(X_samples), item_feature_axes)
        elif akida_model_path:
            feature_explorer_predictions = ei_tensorflow.brainchip.model.predict(akida_model_path, X_samples, len(X_samples))
        else:
            raise Exception('Expecting either a Keras model or an Akida model')

        # Store each prediction with the original sample for the feature explorer
        structured_y = ei_tensorflow.utils.load_y_structured(input_dir, filenames.INPUT_Y_SAMPLES, len(X_samples))
        if len(feature_explorer_predictions) != len(feature_explorer_predictions):
            raise Exception('Expected same number of predictions as labels')
        structured_predictions = []
        for prediction, label in zip(feature_explorer_predictions, structured_y):
            structured_predictions.append({ "prediction": prediction.tolist(), "sample": label })

        return structured_predictions
    except Exception as e:
        print('Failed to generate feature explorer', e, flush=True)

def has_ad_scorer_file(folder: str) -> bool:
    """
    Check if there's a model file for visual AD scoring head, indicating that this is a built-in visual AD model
    """
    folder = Path(folder)
    if not folder.exists() or not folder.is_dir():
        return False

    return any(p.name.startswith('scorer.') for p in folder.iterdir())
...
def get_operator_codes(model: tflite.Model):
    "Extract list of operator builtin codes from tflite model"
    op_codes = []
    for i in range(model.OperatorCodesLength()):
        oc = model.OperatorCodes(i)
        op_codes.append(int(oc.BuiltinCode()))

    return op_codes

def get_subgraph_info(model: tflite.Model, sg_index:int, op_codes:List[int]):
    "Extract architecture info for a single subgraph"
    sg = model.Subgraphs(sg_index)
    sg_name = sg.Name().decode("utf-8") if sg.Name() else f"subgraph_{sg_index}"

    tensors_info = []
    for t_i in range(sg.TensorsLength()):
        t = sg.Tensors(t_i)
        tensors_info.append({
            "name": t.Name().decode("utf-8") if t.Name() else f"tensor_{t_i}",
            "shape": [int(x) for x in t.ShapeAsNumpy()] if t.ShapeLength() > 0 else [],
            "type": int(t.Type()),
        })

    operators_info = []
    for o_i in range(sg.OperatorsLength()):
        op = sg.Operators(o_i)
        opcode_idx = op.OpcodeIndex()
        operators_info.append({
            "opcode": int(op_codes[opcode_idx]),
...
def get_tflite_model_arch(tflite_model: bytes):
    """Extract architecture of a tflite model from a byte buffer as a dict of subgraphs,
    tensors (name, shape, type) and operators (opcode, inputs/outputs), preserving graph order
    and connections. Returns None if parsing fails.
    """
    try:
        model = tflite.Model.GetRootAsModel(tflite_model,0)
        op_codes = get_operator_codes(model)
        architecture = {"subgraphs": []}

        for sg_i in range(model.SubgraphsLength()):
            sg_info = get_subgraph_info(model, sg_i, op_codes)
            architecture["subgraphs"].append(sg_info)
        return architecture
    except Exception as e:
        print("Failed to parse tflite model", flush=True)
        print(e, flush=True)
        return None
...
def get_model_metadata(keras_model: tf.keras.Model,
    ...
    if (curr_model_metadata and 'defaultMinimumConfidenceRating' in curr_model_metadata):
    ...
    if keras_model:
        ...
        if model_float32 is None:
            print("Skipping architecture extraction, no float32 tflite model")
        else:
            arch = get_tflite_model_arch(model_float32)
            metadata['kerasJSON'] = json.dumps(arch, sort_keys=True) if arch is not None else None
        ...
        if (
            ...
            and curr_model_metadata['kerasJSON'] is not None and metadata['kerasJSON'] is not None
    ...
    # Print warning if float32 and int8 model accuracy are > 50% different
    if "float32_perf" in locals() and "int8_perf" in locals():
        if float32_perf and int8_perf:
            if type(float32_perf) is dict and type(int8_perf) is dict:
                float32_accuracy = float32_perf.get('accuracy')
                int8_accuracy = int8_perf.get('accuracy')
                if float32_accuracy and int8_accuracy:
                    if abs(float32_accuracy - int8_accuracy) > 0.5:
                        print(f'Warning: Float32 accuracy '
                              f'({float32_accuracy:3f}) and Int8 accuracy '
                              f'({int8_accuracy:3f}) differ by more than 50%.',
                              flush=True)
    ...
    # Parse any tensorboard logs and convert to a studio-friendly format
    tensorboard_log_dir = '/home/tensorboard_logs'
    if (os.path.exists(tensorboard_log_dir)):
        print('Extracting TensorBoard logs...', flush=True)
        metadata['tensorboardGraphs'] = extract_tensorboard_graphs(tensorboard_log_dir)

        # Also zip the logs archive for download...
        shutil.make_archive(tensorboard_log_dir,
                             'zip', root_dir='/home',
                             base_dir='tensorboard_logs')

        print('Extracting TensorBoard logs OK', flush=True)
...
def extract_tensorboard_graphs(log_dir):
    from tbparse import SummaryReader
    # Read all tensorboard logs, including sub-directories
    tbparser = SummaryReader(log_dir, extra_columns={ 'dir_name' })

    # Helper to determine whether an object is numeric
    def is_number(obj):
        try:
            float(obj)
        except Exception:
    ...
    # Helper to convert a dataframe into raw graph data
    # Dataframe cols: tag, dir_name, value
    # Data with the same tag forms a single graph, where each dir_name is a series.
    def extract(df):
        all_graphs = { }
        if not 'tag' in df:
            return all_graphs

        all_tags = list(set(df['tag'].to_list()))
        for tag in all_tags:
            has_series = False
            series = {}
            all_dirs = list(set(df[df['tag'] == tag]['dir_name'].to_list()))

            # Group directories into separate series for the same graph
            for dir_name in all_dirs:
                all_data = df[(df['tag'] == tag) & (df['dir_name'] == dir_name)]['value'].to_list()
                # Keep only numeric data
                all_data = [x for x in all_data if is_number(x)]
                if (len(all_data) > 0):
                    series[dir_name] = all_data
                    has_series = True

            if has_series:
                all_graphs[tag] = series

        return all_graphs

    # Extract scalars and tensors
    scalar_graphs = extract(tbparser.scalars)
    tensor_graphs = extract(tbparser.tensors)

    # Merge everything. For duplicates, prioritise tensors.
    merged_graphs = {}
    for key in scalar_graphs:
        merged_graphs[key] = scalar_graphs[key]
    for key in tensor_graphs:
        merged_graphs[key] = tensor_graphs[key]
