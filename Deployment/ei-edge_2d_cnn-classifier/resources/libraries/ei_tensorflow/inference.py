...
from ei_shared.types import ClassificationMode, ObjectDetectionDetails, LabelMapMetadata
...
def flatten_model_output(data, log_warning):
    ...
    # Anomaly detection: shape (1,) (single value output).
    if len(data.shape) == 1:
        return data
    ...
    if all(dim == 1 for dim in data.shape[1:]) or all(dim == 1 for dim in data.shape[:-1]):
        if log_warning:
            print('A 1D output is expected for classification, regression, and anomaly detection models. Instead, the output shape was ' + str(data.shape) + '.', flush=True)
    ...
    if log_warning:
        print('EI_LOG_LEVEL=warn A 1D output is expected for classification, regression, and anomaly detection models. Instead, the output shape was ' + str(data.shape) + '.', flush=True)
...
def run_model(mode: ClassificationMode,
    ...
    """
    if mode == 'classification' or mode == 'regression' or mode == 'anomaly-gmm' or mode == 'anomaly' or mode == 'visual-anomaly':
        ...
        if objdet_details.last_layer == 'mobilenet-ssd':
            if not thresholds or not 'min_score' in thresholds:
                raise ValueError(f'threshold "min_score" is required for "{objdet_details.last_layer}"')
            return run_mobilenet_ssd_inference(interpreter, item, specific_input_shape,
                                               thresholds['min_score'], y_data, num_classes)
        elif objdet_details.last_layer == 'yolov2-akida':
            if not thresholds or not 'min_score' in thresholds:
                raise ValueError(f'threshold "min_score" is required for "{objdet_details.last_layer}"')
            return run_akida_yolov2_inference(interpreter, item, specific_input_shape,
                                              thresholds['min_score'], y_data, num_classes, dir_path)
            ...
            return ei_tensorflow.tao_inference.tao_decoding.inference_and_evaluate(interpreter, item, objdet_details,
                                                                specific_input_shape, thresholds['min_score'], y_data,
                                                                num_classes)
            ...
            return run_yolov11_inference(interpreter, item, specific_input_shape,
                                         True, thresholds['min_score'], y_data, num_classes)
        elif objdet_details.last_layer == 'yolov11-abs':
            if not thresholds or not 'min_score' in thresholds:
                raise ValueError(f'threshold "min_score" is required for "{objdet_details.last_layer}"')
            return run_yolov11_inference(interpreter, item, specific_input_shape,
                                         False, thresholds['min_score'], y_data, num_classes)
        elif objdet_details.last_layer == 'paddleocr-detector':
            if not thresholds or not 'min_score_pixel' in thresholds:
                raise ValueError(f'threshold "min_score_pixel" is required for "{objdet_details.last_layer}"')
            if not thresholds or not 'min_score_box' in thresholds:
                raise ValueError(f'threshold "min_score_box" is required for "{objdet_details.last_layer}"')
            if not thresholds or not 'unclip_ratio' in thresholds:
                raise ValueError(f'threshold "unclip_ratio" is required for "{objdet_details.last_layer}"')

            return run_paddleocr_detector_inference(interpreter, item, specific_input_shape, y_data,
                thresholds['min_score_pixel'], thresholds['min_score_box'], thresholds['unclip_ratio'])
        elif objdet_details.last_layer == 'qc-face-det-lite':
            if not thresholds or not 'min_score' in thresholds:
                raise ValueError(f'threshold "min_score" is required for "{objdet_details.last_layer}"')
            return run_qc_face_det_lite_inference(interpreter, item, specific_input_shape,
                                                      thresholds['min_score'], y_data, num_classes)
        else:
            raise ValueError('Invalid object detection last layer "' + str(objdet_details.last_layer) + '"')
    elif mode == 'freeform':
        return run_vector_inference_freeform(interpreter, item, specific_input_shape)
    else:
        raise ValueError('Invalid mode "' + mode + '"')

def invoke(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]', allow_multiple_outputs=False):
...
def run_vector_inference_freeform(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]'):
    """Runs inference that produces a vector output (classification or regression)
    """
    outputs = invoke(interpreter, item, specific_input_shape, allow_multiple_outputs=True)
    return outputs

def run_mobilenet_ssd_inference(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
...
def run_paddleocr_detector_inference(interpreter: tf.lite.Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
                                    y_data: list,
                                    min_score_pixel: float,
                                    min_score_box: float,
                                    unclip_ratio: float):
    """Runs inference that produces an object detection output
    """
    ...
    raw_detections = process_output_paddleocr_detector(output, min_score_pixel, min_score_box, unclip_ratio)
    return compute_performance_object_detection(raw_detections, width, height, y_data, num_classes=1)
...
def process_output_paddleocr_detector(output_data, min_score_pixel, min_score_box, unclip_ratio):
    """Transforms an output tensor into a Python list for object detection
    models.
    Args:
        output_data: The output of the model
    ...
    @dataclass
    class PaddleOCRContour:
        pixels: List[Tuple[int, int]]  # (row, col)

    def find_contours_from_output_f32(m: np.ndarray, threshold: float) -> List[PaddleOCRContour]:
        """4-connected components of pixels where m[r,c] >= threshold."""
        if m.ndim != 2:
        ...
        contours: List[PaddleOCRContour] = []
            ...
            for c in range(cols):
                ...
                while q:
                    ...
                    for k in range(4):
                        ...
                        if visited[nr, nc]:
                            continue
                        if m[nr, nc] < threshold:
                            continue
                        visited[nr, nc] = 1
                        q.append((nr, nc))

                contours.append(PaddleOCRContour(pixels=pixels))

        return contours

    def map_contour_to_bb_f32(
        contour: PaddleOCRContour,
        raw_output_mtx: np.ndarray,
        input_width: int,
        input_height: int,
        unclip_ratio: float,
        ...
        - compute min/max r/c from contour pixels
        - compute mean value inside that rectangle (pre-unclip)
        - unclip by expanding rectangle based on area/perimeter
        - clamp to input bounds
        Returns (x, y, width, height, value)
        """
        # min/max from contour pixels
        rs = [p[0] for p in contour.pixels]
        cs = [p[1] for p in contour.pixels]
        ...
        # mean score inside the pre-unclipped rectangle
        # Clamp slice to raw_output_mtx bounds just in case
        rows, cols = raw_output_mtx.shape
        x0 = max(0, x)
        y0 = max(0, y)
        x1 = min(cols, x + width)
        y1 = min(rows, y + height)

        if x1 <= x0 or y1 <= y0:
            value = 0.0
        else:
            region = raw_output_mtx[y0:y1, x0:x1]
            value = float(region.mean()) if region.size else 0.0

        # unclip distance d
        area = width * height
        perimeter = 2 * (width + height)
        d = int((area * unclip_ratio) / perimeter) if perimeter > 0 else 0
        ...
        if x + width > input_width:
            width = input_width - x
        if y + height > input_height:
            height = input_height - y
    ...
    raw_output_mtx = np.squeeze(output_data)
    input_height = raw_output_mtx.shape[0]
    input_width = raw_output_mtx.shape[1]

    rects = []
    labels = []
    score_res = []

    contours = find_contours_from_output_f32(raw_output_mtx, min_score_pixel)

    for contour in contours:
        x, y, w, h, score = map_contour_to_bb_f32(
            contour=contour,
            raw_output_mtx=raw_output_mtx,
            input_width=input_width,
            input_height=input_height,
            unclip_ratio=unclip_ratio,
        )

        if score < min_score_box:
            continue
    ...
    return list(zip(rects, labels, score_res))
...
def qc_face_det_lite_post_process(hm: np.ndarray, box: np.ndarray, landmark: np.ndarray, threshold: float = 0.2, height:int = 480, width:int = 640):
    '''
    Post process for QC Lightweight Face Detection model
    Args:
        hm: heatmap output from model (NCHW order)
        box: box output from model (NCHW order)
        landmark: landmark output from model (NCHW order)
        threshold: threshold to filter detections
        height: height of input image
        width: width of input image
    Returns:
        List of detections in the format [([ymin, xmin, ymax, xmax], label, score)] (x* and y* are scaled to 0..1)
    '''
    from ei_qai_hub.face_det_lite import detect

    dets = detect(hm, box, landmark, threshold, nms_iou = -1, stride = 8)
    ...
    for n in range(0, len(dets)):
        ...
        if (
            b_Left >= 0
            and b_Top >= 0
            and b_Width - 1 + b_Left < width
            and b_Height - 1 + b_Top < height
        ):
            L = b_Left
            T = b_Top
            W = b_Width
        ...
        res.append(tuple([[T / height, L / width, B / height, R / width], 0, score]))
    return res

def run_qc_face_det_lite_inference(interpreter: Interpreter, item: np.ndarray, specific_input_shape: 'list[int]',
                                        minimum_confidence_rating: float, y_data: list, num_classes):
    if not y_data:
        raise ValueError('y_data must be provided for object detection')
    if not minimum_confidence_rating:
        raise ValueError('minimum_confidence_rating must be provided for object detection')
    height, width, channels = specific_input_shape

    outputs = invoke(interpreter, item, specific_input_shape, allow_multiple_outputs=True)

    # tflite output has different shape than detection code expects (NCHW, pytorch compatible), so we need to transpose the results
    hm = np.expand_dims(np.transpose(outputs[0], (2, 0, 1)), axis=0)  # add batch dimension
    box = np.expand_dims(np.transpose(outputs[1], (2, 0, 1)), axis=0)  # add batch dimension
    landmark = np.expand_dims(np.transpose(outputs[2], (2, 0, 1)), axis=0)  # add batch dimension

    bboxes = qc_face_det_lite_post_process(hm, box, landmark, minimum_confidence_rating, height, width)

    return compute_performance_object_detection(bboxes, width, height, y_data, num_classes)
...
def classify_keras(input_x_file, input_y_file, mode: ClassificationMode, output_file, dir_path,
                   ...
                   thresholds=None,
                   label_map_metadata: Optional[LabelMapMetadata] = None):
    y_true = None
    ...
    is_multilabel = label_map_metadata is not None

    if is_multilabel:
        y_structured = ei_tensorflow.utils.load_y_structured('/', input_y_file, len(input))
        y_true = load_label_map_data(input, y_structured, class_names_testing, label_map_metadata, mode, class_names_training)
        sample_ids = []
        for row in y_structured:
        ...
        # No true labels are expected for anomaly detection.
        # Evaluation in Studio is based on anomaly threshold and expected outcome
        # (see ClassificationSummary)
        y_true = None
    else:
    ...
    if predictions_path is not None:
        if mode == 'object-detection':
            if not thresholds or not 'min_score' in thresholds:
                raise ValueError(f'threshold "min_score" is required for mode "{mode}"')

            with open(predictions_path, 'r') as f:
                pred_y = json.loads(f.read())
                pred_y = map_object_detection_bboxes(pred_y, y_true, thresholds['min_score'], specific_input_shape, num_classes)
        else:
        ...
        # Remove warning about 2D output for visual anomaly models
        # as we don't calculate mode specific metrics for them anyway
        ...
        if use_tflite:
            ...
            for i, item in enumerate(input):
                ...
                pred_y.append(flatten_model_output(single_y_pred, log_warning))
                log_warning = False
    ...
    try:
        if mode == 'regression':
            ...
            if not thresholds or not 'min_confidence_rating' in thresholds:
                raise ValueError(f'threshold "min_confidence_rating" is required for mode "{mode}"')

            training_labels = Labels(class_names_training)
            test_labels = Labels(class_names_testing)
            if is_multilabel:
                # load_label_map_data already mapped idxs
                y_true_one_hot = y_true
                filtered_y_pred_probs = np.stack(pred_y)
            else:
            ...
            evaluator = Evaluator(
                ...
                minimum_confidence_rating=thresholds['min_confidence_rating']
            )
