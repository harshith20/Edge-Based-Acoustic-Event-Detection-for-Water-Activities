...
def calculate_regression_metrics(
    ...
    # Don't include "all" in the grouped metrics by default because there's often no need
    groups_include_all: bool = False,
):
    ...
    def _calc_metrics(yt, yp):
        if len(yt.shape) > 1 and yt.shape[1] > 1:
            # Multi-label:
            # In model testing we sum absolute error over each key, average, then square for the MSE per-sample.
            mse = np.mean(np.mean(np.abs(yt - yp), axis=1) ** 2)
        else:
            # Single-label: use MSE directly
            mse = sklearn_metrics.mean_squared_error(yt, yp)
    ...
    if groups is None:
        ...
        return calculate_grouped_metrics(
            ...
            groups,
            max_groups,
            include_all=groups_include_all,
        )
...
def calculate_classification_metrics(
    ...
    Args:
        y_true_one_hot: ground truth values, in one hot (single label) or multi hot (multi label) encoding.
        y_pred_probs: predictions containing full probability distribution
        num_classes: total number of classes
        groups: (optional) grouping of N elements for y_true, y_pred.
        max_groups: (optional) if set, and groups provided, only use top max_groups by frequency.
        groups_include_all: (optional) Whether to include "all" in grouped metrics
        is_multilabel: (optional) True if this is a multi-label classification task
        minimum_confidence_rating: (optional) Minimum confidence threshold, used for multi-label.
    Returns:
    ...
    if not is_multilabel:
        ...
        # renormalise y_pred_probs. we do this int8 values can
    ...
    def _calc_metrics(y_true_one_hot, y_pred_probs):
        ...
        if is_multilabel:
            ...
            # Multilabel classification report can work with multihot labels
            y_true_labels = y_true_one_hot
            y_pred_labels = np.where(y_pred_probs > confidence_threshold, 1, 0)
        else:
        ...
        metrics["classification_report"] = sklearn_metrics.classification_report(
            ...
            output_dict=True,
            zero_division=0,
            labels=labels if is_multilabel else None,
        )
        ...
        if is_multilabel:
            m_loss = []
            for ix in range(0, y_true_labels.shape[1]):
                x = sklearn_metrics.log_loss(
                    y_true_labels[:, ix], y_pred_probs[:, ix], labels=[0, 1]
                )
                m_loss.append(x)
            # Average per-class log loss to match binary crossentropy loss used during training
            metrics["loss"] = np.mean(m_loss)

            # Accuracy is hamming loss (matches model testing + the "binary_accuracy" metric used when training).
            # For hamming loss, 0 is the best score, so do 1-loss to get accuracy score.
            metrics["classification_report"]["accuracy"] = (
                1 - sklearn_metrics.hamming_loss(y_true_labels, y_pred_labels)
            )
        else:
        ...
        for key in ["precision", "recall", "f1-score"]:
            metrics["weighted_average"][key] = metrics["classification_report"][
                "weighted avg"
            ][key]
...
def _coco_map_calculation_from_studio(
    ...
    if len(y_true_bbox_labels) != len(prediction):
        raise Exception(
            "Expected to have same number of y_true and y_pred"
    ...
    for instance_y_true, instance_y_pred in zip(y_true_bbox_labels, prediction):
        ...
        for bbox_label_score in instance_y_true:
            ...
            gt.append(
                [bbox.x0, bbox.y0, bbox.x1, bbox.y1, bbox_label_score.label, 0, 0]
            )
        ...
        for bbox_label_score in instance_y_pred:
            ...
            curr_ps.append(
                [
                    ...
                    bbox.x1,
                    bbox.y1,
                    bbox_label_score.label,
                    bbox_label_score.score,
                ]
            )
...
def calculate_fomo_metrics(
    ...
    Args:
        y_true_labels: ground truth labels with same shape as y_pred_labels
        y_pred_labels: predicted labels with same shape as y_true_labels
        num_classes: total number of classes.
        groups: (optional) grouping of N elements for y_true, y_pred.
        max_groups: (optional) if set, and groups provided, only use top max_groups by frequency.
    Returns:
