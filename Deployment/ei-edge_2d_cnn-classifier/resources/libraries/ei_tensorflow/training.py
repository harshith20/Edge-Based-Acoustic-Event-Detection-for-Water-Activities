...
import ei_shared.filenames as filenames
from ei_shared.parse_train_input import load_label_map_data
...
def _get_npy_header(array: np.ndarray, new_length: int) -> Dict[str, Any]:
    """Generate a .npy header for a rewritten array with a different row count."""
    new_shape = (new_length,) + array.shape[1:]
    return {'descr': fmt.dtype_to_descr(array.dtype), 'fortran_order': False, 'shape': new_shape}

def save_to_npy(array: np.ndarray, indexes: Collection[int], file_path: str) -> None:
    """Save a subset of array rows to a .npy file in the order specified by indexes."""
    header = _get_npy_header(array, len(indexes))
    with open(file_path, 'wb') as f:
        fmt.write_array_header_2_0(f, header)
        for ix in indexes:
            f.write(array[ix].tobytes('C'))

def save_full_array_to_npy(array: np.ndarray, file_path: str) -> None:
    """Save an array to .npy via save_to_npy to keep one code path for header writing."""
    save_to_npy(array, range(len(array)), file_path)

def _get_split_output_paths(output_dir: str) -> Tuple[str, str, str, str, str, str]:
    """Return the standard file paths used for split dataset artifacts."""
    return (
        os.path.join(output_dir, filenames.OUTPUT_X_SPLIT_TRAIN),
        os.path.join(output_dir, filenames.OUTPUT_X_SPLIT_TRAIN_RAW),
        os.path.join(output_dir, filenames.OUTPUT_X_SPLIT_TEST),
        os.path.join(output_dir, filenames.OUTPUT_Y_SPLIT_TRAIN),
        os.path.join(output_dir, filenames.OUTPUT_Y_SPLIT_TEST),
        os.path.join(output_dir, filenames.OUTPUT_SAMPLE_ID_DETAILS),
    )

def _write_split_data(output_dir: str,
                      y_type: str,
                      X_train: Any,
                      X_test: Any,
                      Y_train: Any,
    ...
    """Persist split arrays/metadata in the shared format used by training helpers."""
    X_train_output_path, X_train_raw_output_path, X_test_output_path, Y_train_output_path, \
        Y_test_output_path, sample_id_details_output_path = _get_split_output_paths(output_dir)

    save_full_array_to_npy(X_train, X_train_output_path)
    save_full_array_to_npy(X_test, X_test_output_path)

    sample_id_details = {
        'train': train_sample_ids.tolist(),
        'validation': test_sample_ids.tolist(),
    }
    with open(sample_id_details_output_path, 'w') as f:
        json.dump(sample_id_details, fp=f)

    if X_train_raw is not None:
        save_full_array_to_npy(X_train_raw, X_train_raw_output_path)
    ...
    if y_type == 'structured':
        ...
        save_full_array_to_npy(Y_train, Y_train_output_path)
        save_full_array_to_npy(Y_test, Y_test_output_path)

    return load_split_and_shuffled_data(output_dir, y_type)

def load_explicit_validation_split(y_type, classes, classes_values, mode, input_dir, output_dir,
                                   split_raw_data=False,
                                   model_input_shape=None,
                                   label_map_metadata=None):
    """Load a pre-defined train/validation split and persist it in the standard split layout."""

    X_train = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_TRAIN_FEATURES))
    X_test = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_VALIDATION_FEATURES))
    X_train_raw = None
    ...
    if y_type == 'structured':
        ...
        Y_train = ei_tensorflow.utils.load_y_structured(input_dir, filenames.INPUT_Y_TRAIN, len(X_train))
        Y_test = ei_tensorflow.utils.load_y_structured(input_dir, filenames.INPUT_Y_VALIDATION, len(X_test))
        train_sample_ids = np.array([r['sampleId'] for r in Y_train])
        test_sample_ids = np.array([r['sampleId'] for r in Y_test])

        if label_map_metadata is not None:
            Y_train = load_label_map_data(X_train, Y_train, classes_values, label_map_metadata, mode)
            Y_test = load_label_map_data(X_test, Y_test, classes_values, label_map_metadata, mode)
            y_type = 'npy'

    elif y_type == 'npy':
        Y_train_file = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_Y_TRAIN))
        Y_test_file = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_Y_VALIDATION))
        Y_train = Y_train_file[:,0]
        Y_test = Y_test_file[:,0]
        train_sample_ids = Y_train_file[:,1]
        test_sample_ids = Y_test_file[:,1]

        if split_raw_data:
            X_train_raw = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_TRAIN_RAW))

        if mode == 'classification':
            Y_train = tf.keras.utils.to_categorical(Y_train - 1, num_classes=classes)
            Y_test = tf.keras.utils.to_categorical(Y_test - 1, num_classes=classes)
        elif mode == 'regression':
            Y_train = np.array([float(classes_values[y - 1]) for y in Y_train])
            Y_test = np.array([float(classes_values[y - 1]) for y in Y_test])
    ...
    if model_input_shape:
        X_train = X_train.reshape(tuple([X_train.shape[0]]) + model_input_shape)
        X_test = X_test.reshape(tuple([X_test.shape[0]]) + model_input_shape)

    return _write_split_data(output_dir, y_type, X_train, X_test, Y_train, Y_test,
                             train_sample_ids, test_sample_ids, X_train_raw)

def create_explicit_validation_split(y_type, classes, classes_values, mode, input_dir, output_dir,
                                     split_raw_data=False,
                                     model_input_shape=None,
                                     label_map_metadata=None):
    """Backward-compatible wrapper for load_explicit_validation_split."""
    return load_explicit_validation_split(
        y_type, classes, classes_values, mode, input_dir, output_dir,
        split_raw_data=split_raw_data,
        model_input_shape=model_input_shape,
        label_map_metadata=label_map_metadata)
...
def split_and_shuffle_data(y_type, classes, classes_values, mode, seed, input_dir, output_dir, test_size=0.2,
                           split_raw_data=False,
    ...
    if y_type == 'structured':
        ...
        X = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_TRAIN_FEATURES))
        Y = ei_tensorflow.utils.load_y_structured(input_dir, filenames.INPUT_Y_TRAIN, len(X))

        sample_ids = np.array([r['sampleId'] for r in Y])

        if label_map_metadata is not None:
            Y = load_label_map_data(X, Y, classes_values, label_map_metadata, mode)
            # y is now loaded as npy; treat it as npy rather than structured data
            y_type = 'npy'

    elif y_type == 'npy':
        X = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_TRAIN_FEATURES))
        Y_file = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_Y_TRAIN))
        Y = Y_file[:,0]
        sample_ids = Y_file[:,1]

        if split_raw_data:
            X_raw = np_load_file_auto_mmap(os.path.join(input_dir, filenames.INPUT_X_TRAIN_RAW))
    ...
    X_train_idxs_assigned = []
    X_test_idxs_assigned = []
    X_idxs_unassigned = []

    if custom_validation_split:
        validation_split_metadata = ei_tensorflow.utils.load_validation_split_metadata(input_dir, filenames.INPUT_CUSTOM_VALIDATION_SPLIT)
        if validation_split_metadata is not None:
            ...
            for ix, sample_id in enumerate(sample_ids):
                ...
                if sample_id not in validation_split_metadata:
                    X_idxs_unassigned.append(ix)
                    continue

                if validation_split_metadata[sample_id] == 'train':
                    X_train_idxs_assigned.append(ix)
                elif validation_split_metadata[sample_id] == 'validation':
                    X_test_idxs_assigned.append(ix)
                else:
                    X_idxs_unassigned.append(ix)
        else:
            custom_validation_split = False
    ...
    if not custom_validation_split:
        X_idxs_unassigned = list(range(len(X)))

    # Perform train/test split on any samples not explicitly assigned a category
    if len(X_idxs_unassigned) > 0:
        if test_size == 0:
            train_idxs = X_idxs_unassigned
            test_idxs = []
        else:
            train_idxs, test_idxs = train_test_split(X_idxs_unassigned,
                                                    test_size=test_size,
                                                    random_state=seed,
    ...
    # Merge groups
    train_idxs = train_idxs + X_train_idxs_assigned
    test_idxs = test_idxs + X_test_idxs_assigned

    # If we had a custom validation split, shuffle again
    if custom_validation_split:
        train_idxs = shuffle(train_idxs, random_state=seed)
        test_idxs = shuffle(test_idxs, random_state=seed)
        if len(test_idxs) == 0:
            raise Exception('ERROR: No samples in validation set! '
                            '\nPlease check your custom validation split. '
                            '\nIf you wanted to set validation set explictly '
                            'via "Split train/validation set on metadata key" '
                            'you need to change validation set size to 0')


    save_to_npy(X, train_idxs, X_train_output_path)
    save_to_npy(X, test_idxs, X_test_output_path)

    # write lookup for train/test rows to sample id so we can join on meta data
    # as required later
    sample_id_details = {
        'train': sample_ids[train_idxs].tolist(),
        'validation': sample_ids[test_idxs].tolist()
    }
    with open(sample_id_details_output_path, 'w') as f:
        json.dump(sample_id_details, fp=f)
    ...
    if split_raw_data:
        ...
        save_to_npy(X_raw, train_idxs, X_train_raw_output_path)
    ...
    if y_type == 'structured':
        ...
        save_to_npy(Y, train_idxs, Y_train_output_path)
        save_to_npy(Y, test_idxs, Y_test_output_path)

    return load_split_and_shuffled_data(output_dir, y_type)

def load_split_and_shuffled_data(data_directory, y_type):
    X_train_output_path, X_train_raw_output_path, X_test_output_path, Y_train_output_path, \
        Y_test_output_path, sample_id_details_output_path = _get_split_output_paths(data_directory)
    ...
    X_test = None
    if os.path.exists(X_test_output_path):
    ...
    if y_type == 'structured':
        ...
        if os.path.exists(Y_test_output_path):
            Y_test = np_load_file_auto_mmap(Y_test_output_path)

    try:
        with open(sample_id_details_output_path, 'r') as f:
            sample_id_details = json.load(f)
    except FileNotFoundError:
        sample_id_details = None

    return X_train, X_test, Y_train, Y_test, X_train_raw, sample_id_details
...
def get_dataset_from_folder(input,
    ...
    classes = len(ei_tensorflow.training.get_output_classnames(input))
    mode = input.mode
    explicit_validation_split = input.explicitValidationSplit if hasattr(input, 'explicitValidationSplit') else False
    custom_validation_split = input.customValidationSplit if hasattr(input, 'customValidationSplit') else False
    label_map_metadata = input.labelMap if hasattr(input, 'labelMap') else None

    # if we have split data already in the out directory, then use that
    if (os.path.exists(os.path.join(data_directory, filenames.OUTPUT_X_SPLIT_TRAIN))):
        # By this point, structured data has been parsed and we can load it directly from npy
        if label_map_metadata is not None:
            y_type = 'npy'
        X_train, X_test, Y_train, Y_test, X_train_raw, sample_id_details = load_split_and_shuffled_data(data_directory, y_type)
        ...
        if explicit_validation_split:
            print('Using explicit validation split...', flush=True)
            X_train, X_test, Y_train, Y_test, X_train_raw, sample_id_details = load_explicit_validation_split(
                y_type, classes, classes_values, mode, data_directory,
                output_dir='/tmp',
                split_raw_data=hasattr(input, 'onlineDspConfig'),
                model_input_shape=input_shape,
                label_map_metadata=label_map_metadata)
            print('Using explicit validation split OK', flush=True)
        else:
            ...
            X_train, X_test, Y_train, Y_test, X_train_raw, sample_id_details = split_and_shuffle_data(
                ...
                # If the split has not already been done, save the output to /tmp since
                # we will just be loading it immediately. NB this is designed to be run in
                # a container, meaning /tmp is ephemeral.
                output_dir='/tmp',
        ...
        # write sample_id_details mapping from where split_and_shuffle_data operates to the block
        # data directory to ensure it's available for metrics processing.
        if sample_id_details is not None:
            with open(os.path.join(data_directory, filenames.OUTPUT_SAMPLE_ID_DETAILS), 'w') as f:
                json.dump(sample_id_details, fp=f)
...
def get_samples(dir_path):
    ...
    X_samples_path = os.path.join(dir_path, filenames.INPUT_X_SAMPLES)
    X_samples = None
    Y_samples_path = os.path.join(dir_path, filenames.INPUT_Y_SAMPLES)
    Y_samples = None
    has_samples = False
    try:
        x_tmp_file = os.path.join('/tmp', filenames.INPUT_X_SAMPLES)
        shutil.copyfile(X_samples_path, x_tmp_file)
        X_samples = np_load_file_auto_mmap(x_tmp_file)

        y_tmp_file = os.path.join('/tmp', filenames.INPUT_Y_SAMPLES)
        if ei_tensorflow.utils.is_y_structured(Y_samples_path):
            # Use placeholder unstructured data (this isn't used downstream for structured data, but we still
            # need to create the Dataset object from it)
            temp_data = np.zeros((len(X_samples), 4))
            np.save(y_tmp_file, temp_data)
        else:
...
def load_samples(dir_path):
    ...
    has_samples = False
    try:
        x_tmp_file = os.path.join(dir_path, filenames.INPUT_X_SAMPLES)
        X_samples = np_load_file_auto_mmap(x_tmp_file)

        y_tmp_file = os.path.join(dir_path, filenames.INPUT_Y_SAMPLES)
        if ei_tensorflow.utils.is_y_structured(y_tmp_file):
            # Use placeholder unstructured data (this isn't used downstream for structured data, but we still
            # need to create the Dataset object from it)
            Y_samples = np.zeros((len(X_samples), 4))
        else:
            Y_samples = np_load_file_auto_mmap(y_tmp_file)
...
def get_callbacks(dir_path, mode, best_model_path, object_detection_last_layer: ObjectDetectionLastLayer,
                  is_enterprise_project, max_training_time_s, max_gpu_time_s, enable_tensorboard=True):
    callbacks = []
    if mode == 'object-detection':
        if (object_detection_last_layer == 'fomo'):
    ...
    tensorboard_log_dir = os.path.join(dir_path, 'tensorboard_logs')
...
def print_training_time_exceeded(is_enterprise_project, max_training_time_s, total_time):
    ...
    if (is_enterprise_project):
        ...
        print('See https://docs.edgeimpulse.com/knowledge/guides/lowering-compute-time on tips to lower your compute time requirements.')
        print('')
...
def get_output_classnames(input):
    """Get in-order list of class names associated with the output of the model.

    Args:
        input: Model training input.json
    """
    if hasattr(input, 'labelMap'):
        # Multi-label dataset
        if input.mode == 'regression':
            return input.labelMap.columns
        else:
